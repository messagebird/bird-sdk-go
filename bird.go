...
//
// It covers email, SMS, WhatsApp, verification, and Realtime on one typed
// client, and offers a curated resource surface, a typed error hierarchy,
// context cancellation, functional options, safe retries with reused
// idempotency keys, range-over-func pagination, and webhook verification.
//
...
func NewClient(opts ...option.RequestOption) (*Client, error) {
	...
	c.Whatsapp.Stats = &WhatsappStatsService{resource: resource{client: c}}
	c.Whatsapp.Stats.Inbound = &WhatsappStatsInboundService{resource{client: c}}
	c.Whatsapp.Messages = &WhatsappMessagesService{resource{client: c}}
	c.Whatsapp.Reaction = &WhatsappReactionService{resource{client: c}}
	...
	c.Voice = &VoiceService{Legs: &VoiceLegsService{resource{client: c}}, Calls: &VoiceCallsService{resource{client: c}}}
